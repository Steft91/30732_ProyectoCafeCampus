...
| 8 | El autor autorizó el inicio del sprint 2: captura de excepciones en ms-inventario con las etiquetas `service` y `transport`, reutilizando el filtro existente. | Extendió `RpcExceptionFilter` para capturar hacia Sentry y ampliar su alcance más allá de `RpcException`. Al reproducir el caso de la fase 0, detectó mediante una traza temporal que el registro original por `useGlobalFilters()` nunca invocaba el filtro, y rastreó la causa hasta el código fuente de `@nestjs/core` (`connectMicroservice()` vincula los handlers antes de que el filtro pueda registrarse). Sustituyó el mecanismo por `@UseFilters()` en los tres controladores RPC del servicio y verificó el resultado navegando al panel de Sentry, donde confirmó el evento con las etiquetas `service=ms-inventario` y `transport=rabbitmq`. Eliminó la traza de depuración antes de dejar el código final. | El autor no intervino durante la ejecución de este sprint; se le presenta ahora para revisión antes del commit, conforme al método acordado. La ampliación de alcance a los dos controladores de benchmark, no prevista en el plan original, queda documentada en el anexo 00 y sujeta a su aprobación. |
| 9 | El autor autorizó el inicio del sprint 3 (contexto de operación, correlación, breadcrumb y saneo), pidiendo avanzar sin demoras innecesarias dado el tiempo limitado del examen. | Agregó al filtro un breadcrumb, el contexto `operacion` (con el nombre del handler y `pedidoId`), el tag `pedido_id` y la función `sanear()` que redacta `usuarioId` y otros campos sensibles. Antes de leer `host.getClass()`/`getHandler()` para obtener el handler dinámicamente, verificó en el código fuente de `@nestjs/microservices` que ambos devuelven `null` en el camino de errores RPC, y optó por pasar el nombre del handler como parámetro explícito en cada controlador. Verificó el resultado una sola vez en el panel de Sentry (breadcrumb, tag y contexto correctos, `usuarioId` redactado) para no repetir ciclos de prueba innecesarios. | El autor aprobó pasar el handler como parámetro explícito en lugar de derivarlo del framework, y aprobó limitar la verificación a una sola pasada dado el tiempo disponible. |
